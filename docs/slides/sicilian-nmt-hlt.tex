    ...
    \begin{column}{0.38\textwidth}\footnotesize
      \begin{itemize}
        ...
        \item The pretrained model wins decisively: \textbf{+18 BLEU} zero-shot over the best
              from-scratch model.
        \item LoRA fine-tuning adds \textbf{+2.3} on top, to \textbf{31.43} --- past the
...
\begin{frame}{Conclusion \& outlook}
  \begin{itemize}
    ...
    \item On our harder literary test set we reach \textbf{31.43} (scn$\to$en) from a 5.27
          floor --- above the published \emph{baseline}, though Wdowiak's full reverse-training
          system is well ahead on every direction.
    \item \textbf{To close the method gap}: back-translation (in progress) and reverse-training
          on top of NLLB. The likely \emph{residual} is then data quality --- his hand-curated
          \emph{Arba Sicula} corpus vs our automatic extraction --- which motivates
          \textbf{collaborating} rather than competing.
    \item Also: Italian bridging (done) and serving as a web app + \textbf{Telegram bot}.
  \end{itemize}
  \vfill
