...
\setbeamertemplate{headline}{%
  \vskip1.2mm%
  \hbox to \paperwidth{\hspace{9mm}{\scriptsize\color{tan}Sicilian--English Neural Machine Translation}%
...
\title{\textbf{Sicilian--English Neural Machine Translation}}
\author{Donato Meoli}
\date{}
...
\begin{frame}[plain]
  ...
  {\color{tan}\rule{2pt}{2.4cm}}\hspace{3mm}
  \begin{minipage}{0.82\textwidth}\raggedright
    {\Huge Sicilian--English\\ Neural Machine\\ Translation}\\[3mm]
...
% ==================================================================== problem
\begin{frame}{A translator for Sicilian}
  \begin{columns}[t]
    \begin{column}{0.62\textwidth}
      \begin{itemize}
        \item \textbf{Google}, \textbf{DeepL}, \textbf{Bing} do not translate Sicilian ---
              yet it is a full \textbf{Romance language}, the first literary standard in
              Italy (Sicilian School of poets, 13\textsuperscript{th} c.), spoken daily by
              millions.
        \item It is \textbf{low-resource}: the bottleneck was never the model but the
              \emph{parallel text}. Wdowiak's \emph{Tradutturi Sicilianu} (2021) was the
              first Sicilian NMT, trained on hand-aligned \emph{Arba Sicula} journal text.
        \item \textbf{This work} rebuilds the whole thing: an \emph{automated} extraction
              pipeline (no Perl, no hunalign), a reproducible dataset and evaluation, the
              paper's recipe re-derived, and modern \textbf{pretrained} models on top.
      \end{itemize}
    \end{column}
    \begin{column}{0.38\textwidth}
      \centering
      \includegraphics[width=\textwidth]{browser-white-box-new_v1.png}\\[1mm]
      {\scriptsize the target: literary Sicilian$\,\leftrightarrow\,$English.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Injecting grammar --- desinence-biased subwords}
  ...
  \end{itemize}
  \vfill
  {\footnotesize\color{gray} The Napizia Perl tokenizer, re-implemented in Python and verified
  identical, so the whole recipe runs Perl-free.}
\end{frame}
...
\begin{frame}{Bidirectional fine-tune --- one model, both directions}
  \begin{itemize}
    \item Fine-tuning only scn$\to$en leaves the reverse direction weak. Instead train
    ...
    \item One adapter then serves \textbf{scn$\to$en} \emph{and} \textbf{en$\to$scn}; the shared
          encoder/decoder regularise each other.
    \item Result (preview): en$\to$scn \textbf{nearly doubles} (9.89 $\to$ 18.65 BLEU) at
          \emph{no cost} to scn$\to$en --- and we now have a usable reverse model, which we need next.
  \end{itemize}
\end{frame}
  ...
  \begin{columns}[c]
    \begin{column}{0.6\textwidth}
      ...
      \begin{tabular}{lrr}
        ...
        \;+ lever D (lemma factors)    & 10.85 & 35.22 \\\midrule
        NLLB-1.3B, zero-shot           & 29.00 & 55.23 \\
        NLLB-1.3B, LoRA bidirectional  & \textbf{31.33} & \textbf{56.92} \\\bottomrule
      \end{tabular}
    \end{column}
    \begin{column}{0.4\textwidth}\footnotesize
      \begin{itemize}
        \item Each Sockeye lever \textbf{stacks}: $5.54\!\to\!10.85$ on CPU alone.
        \item The pretrained model wins decisively: \textbf{+18 BLEU} zero-shot over the best
              from-scratch model.
        \item LoRA adds \textbf{+2.3} on top, to \textbf{31.33}.
        \item Reverse direction en$\to$scn: $9.89\!\to\!\textbf{18.65}$ after the bidirectional
              fine-tune.
      \end{itemize}
    \end{column}
  \end{columns}
