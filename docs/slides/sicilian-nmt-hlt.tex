...
\begin{frame}[plain]
  ...
  \begin{minipage}{0.82\textwidth}\raggedright
    {\Huge Sicilian--English--Italian\\[1mm]Neural Machine Translation}\\[3mm]
...
% ==================================================================== motivation
\begin{frame}{Why is there no \emph{good} Sicilian translator?}
  \begin{itemize}
    \item For years Sicilian was \textbf{absent} from the major services. It is now included in
          massively multilingual models (\textbf{NLLB-200}) --- but the coverage is
          \textbf{low-quality}: learned from non-standardised web text, not the traditional
          orthography. The barrier shifted from \emph{absence} to \emph{quality}.
...
% ================================================================ PREPROCESSING
\sectiondivider{Preprocessing\\ the Sicilian\\ language}{%
  \begin{itemize}
    \item No single orthography $\to$ \emph{Standard Sicilian}
    \item ASCII-folding (drop circumflex accents)
    \item Uncontraction (prepositions, \emph{aviri}, clitics)
  \end{itemize}}{%
  The orthographic standard of \emph{Arba Sicula}; rules following Wdowiak's
  \emph{Tradutturi} text preprocessing. Done before any model sees the data.}

\begin{frame}{Why NLLB's Sicilian is low quality}
  \begin{columns}[t]
    \begin{column}{0.54\textwidth}
      \begin{itemize}
        \item About \textbf{one third} of the ``Sicilian'' NLLB collected is local
              \textbf{dialect}; some is actually \textbf{Corsican}, and few sentences carry a
              valid translation.
        \item Why? Romance languages are so close that a \textbf{Corsican$\to$English} model
              \emph{appears} to work on Sicilian --- so mining by surface-form cues admits
              Corsican and dialect as ``Sicilian''.
        \item Multilingual encoders organise around \textbf{surface form}, not a shared
              interlingua (Verma et al., 2026) --- a Corsican-looking surface form yields
              Corsican output.
      \end{itemize}
    \end{column}
    \begin{column}{0.46\textwidth}
      \footnotesize
      \textbf{Our evidence (LaBSE on FLORES-200):}\\[1mm]
      \begin{tabular}{lr}
        scn$\to$en retrieval P@1        & \textbf{99\%} \\
        \quad vs \emph{supported} Italian & 100\% \\
        Sicilian's nearest language     & \textbf{Italian} \\
        \quad (18\%, 2$\times$ the next Romance) & \\
      \end{tabular}
      \\[2.5mm]
      \textbf{Alignability tracks standardness:}\\[1mm]
      \begin{tabular}{lcc}
                       & cosine & recovered \\
  ...
  \end{columns}
  \vfill
  {\footnotesize\color{gray} Sicilian sits in LaBSE's Italian/Romance region: the same
  proximity lets our aligner work on an officially \emph{unsupported} language, but offers no
  protection against near-language contamination. Curating toward the literary standard is what
  fixes it --- which motivates the next step.}
\end{frame}

\begin{frame}{Standardising the Sicilian language}
  \begin{columns}[t]
    \begin{column}{0.56\textwidth}
      \begin{itemize}
        \item Sicilian has \textbf{no single official orthography}: many local varieties,
              writers omit accents, several spellings per word.
        \item \emph{Arba Sicula} developed a \textbf{Standard Sicilian}; we normalise every
              sentence to it \emph{before} training --- the data-quality step Wdowiak did
              \textbf{by hand}, here automated and Perl-free.
        \item Cuts vocabulary sparsity (decisive in low-resource) and aligns noisy web-mined
              text with the literary standard.
      \end{itemize}
    \end{column}
    \begin{column}{0.44\textwidth}
      \footnotesize
      \textbf{Standardisation (sample rules):}\\[1mm]
      \begin{tabular}{rcl}
        cchi\`u & $\to$ & chi\`u \\
        io / iu / eu & $\to$ & jo \\
        non & $\to$ & nun \\
        'un, 'unn & $\to$ & nun \\
        bell\textit{-} & $\to$ & bedd\textit{-} \\
        pri / pir & $\to$ & pi \\
        picch\`i & $\to$ & pirch\`i \\
      \end{tabular}
    \end{column}
  \end{columns}
  \vfill
  {\footnotesize\color{gray} The standard is Cipolla's, augmented for MT clarity: strict
  \textbf{L} on articles (\emph{lu/la/li}), \textbf{h} on \emph{aviri} (\emph{haiu, havi,
  hannu}), \textbf{CI} for \c{c}i (\emph{\c{c}iuri$\to$ciuri}), and \textbf{R} kept before a
  clitic (\emph{farlu}).}
\end{frame}

\begin{frame}{Normalisation rules: ASCII-fold + uncontraction}
  \begin{columns}[t]
    \begin{column}{0.5\textwidth}
      \footnotesize
      \textbf{ASCII-fold} --- drop circumflex accents (writers omit them anyway):
      \[ \hat a\!\to\!a,\ \ \hat e\!\to\!e,\ \ \hat\imath\!\to\!i,\ \ \hat o\!\to\!o,\ \ \hat u\!\to\!u \]
      \textbf{Uncontract} preposition\,+\,article (a/cu/di/pi/nta\dots):
      \begin{tabular}{rcl@{\quad}rcl}
        \^o & $\to$ & a lu & d\^u & $\to$ & di lu \\
        c\^a & $\to$ & cu la & nt\^o & $\to$ & nta lu \\
      \end{tabular}
    \end{column}
    \begin{column}{0.5\textwidth}
      \footnotesize
      \textbf{Uncontract} the verb \emph{aviri} and clitic pronouns:
      \begin{tabular}{rcl@{\quad}rcl}
        h\^a \{v\} & $\to$ & havi a \{v\} & & & \\
        m\^u & $\to$ & mi lu & t\^a & $\to$ & ti la \\
      \end{tabular}
      \vspace{2mm}
      \textbf{Before:} \texttt{ntô libbru dû nannu}\\
      \textbf{After:} \;\, \texttt{nta lu libbru di lu nannu}\\[1.5mm]
      Perl-free in \texttt{sicilian\_tok.py} (verified byte-identical to the Napizia tokenizer).
    \end{column}
  \end{columns}
  \vfill
  {\footnotesize\color{gray} \textbf{Ablation (NLLB, scn$\to$en):} raw 30.6 $\to$ \emph{std}
  \textbf{31.0} $\to$ \emph{full} 31.0 BLEU. Light standardisation helps a little; the
  aggressive ``full'' preprocessing does \emph{not} beat it --- a pretrained tokenizer wants
  un-normalised input. Orthography is a \emph{minor} lever; the residual is data curation.}
\end{frame}
...
\begin{frame}{Subword segmentation --- BPE}
  ...
  \begin{itemize}
    ...
    \item Wdowiak's example --- \emph{Carinisi are dogs!} tokenizes to
          \texttt{car@@ in@@ isi are dogs !}; biasing the merges toward textbook desinences cut
          the vocabulary from \textbf{14{,}000 to 2{,}000} units and gave his largest BLEU gains.
...
\begin{frame}{NLLB-200 --- a massively multilingual Transformer}
  \begin{columns}[t]
    \begin{column}{0.58\textwidth}
      \footnotesize
      One encoder--decoder Transformer trained on \textbf{200 languages}, and
      \textbf{Sicilian (\texttt{scn\_Latn}) is supported} among them. The decoder is steered to the target
  ...
  \begin{columns}[t]
    ...
    \begin{column}{0.42\textwidth}
      \begin{itemize}
        ...
        \item \textbf{Our result}: 7{,}498 synthetic pairs lift scn$\to$en only
              $31.43\!\to\!\textbf{31.6}$ ($+0.17$).
        \item \textbf{Why so small?} His gain came from \textbf{$\sim$550k} back-translations;
              our in-domain pool is just \textbf{7.6k}. The lever is \emph{data-limited} ---
              not a method failure but a \textbf{data} ceiling.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
    ...
    \begin{column}{0.38\textwidth}\footnotesize
      \begin{itemize}
        ...
        \item The pretrained model wins decisively: \textbf{+18 BLEU} zero-shot over the best
              from-scratch model.
        \item LoRA fine-tuning adds \textbf{+2.3} (to 31.43); back-translation a further
              $+0.17$ to \textbf{31.6} --- past the published baseline on our harder test.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Adding Italian --- one trilingual model}
  \begin{columns}[c]
    \begin{column}{0.6\textwidth}
      \centering
      \includegraphics[width=\textwidth]{bleu_trilingual.png}
    \end{column}
    \begin{column}{0.4\textwidth}\footnotesize
      \begin{itemize}
        \item A single multilingual LoRA adapter fine-tuned on four directions
              (scn$\leftrightarrow$en, it$\leftrightarrow$scn).
        \item \textbf{Italian is strong}: scn$\to$it \textbf{43.5}, it$\to$scn
              $17.6\!\to\!\textbf{27.0}$ --- a close, high-resource Romance neighbour
              (it--scn test is WikiMatrix-domain).
        \item Small cost to scn$\leftrightarrow$en vs the dedicated bidirectional model
              ($30.8/17.5$ vs $31.4/18.7$): ship the trilingual all-rounder or the
              specialised pair.
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}{We do not beat the state of the art (yet)}
  \begin{columns}[c]
    \begin{column}{0.58\textwidth}
      \centering
      \includegraphics[width=\textwidth]{bleu_vs_sota.png}
    \end{column}
    \begin{column}{0.42\textwidth}\footnotesize
      \begin{itemize}
        \item The SOTA is Wdowiak's \textbf{reverse-training} system; it leads on \emph{every}
              direction (his Italian reaches 61--63).
        \item We only clear his \emph{baseline} on Sc$\to$en (31.4 vs 29.1), on a harder test;
              elsewhere we trail.
        \item His full recipe builds a custom pretrained model from \textbf{tens of millions}
              of pairs (forward + back-translation, 3 stages) --- not feasible to reproduce on
              a single GPU.
        \item \textbf{But NLLB-200 already \emph{is} that pretraining}: our run $\approx$ his
              stage-3 fine-tune. Adding back-translation + multilingual closes the
              \emph{method} gap; the residual is then \textbf{data quality}.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Conclusion \& outlook}
  \begin{itemize}
    ...
    \item On our harder literary test set we reach \textbf{31.6} (scn$\to$en) from a 5.27
          floor --- above the published \emph{baseline}, though Wdowiak's full reverse-training
          system is well ahead on every direction.
    \item \textbf{Method levers, all tested on NLLB}: multilingual Italian bridge (done),
          orthographic normalisation ($+0.4$), back-translation ($+0.17$, capped by our 7.6k
          monolingual pool vs his $\sim$550k). Each helps little --- the gap is \emph{not}
          method.
    \item \textbf{So the residual is data}: his hand-curated \emph{Arba Sicula} corpus, far more
          back-translation, and an in-domain test set --- a concrete case for
          \textbf{collaborating} with the author rather than competing. Also: a web app +
          \textbf{Telegram bot}.
  \end{itemize}
  \vfill
